...
struct Line{                                   //解析.sh文件中的每一行命令，每一行用一个struct Line来保存
    char* originalCmd;
    char* briefCmd;
    int error;
    int blocked;
    char* variableNames[2];
    char* valuesOfVariable[2];
    int valueIsNum[2];
    int compareMode;
    int executable;
    int type;
...
char* removeBlank(char* origin);
struct Line parseLineStruct(char* originCmd);
int checkBranchType(char* line);
void getVariableAndCompareMode(struct Line* line);
void getValueOfVariables(struct Line* line);
void compareValues(struct Line* line);

char* substring(char* src, int begin, int end){
    char* dest=(char*)malloc(sizeof (char)*(end-begin+1));
...
struct Line parseLineStruct(char* originCmd){
    struct Line line;
    //printk("originCmd: %s\n",originCmd);
        line.originalCmd=substring(originCmd,0,strlen(originCmd));
        line.briefCmd=removeBlank(originCmd);
        line.type=checkBranchType(line.briefCmd);
        line.closed=-1;
        line.blocked=0;
        line.error=0;
        line.executable=0;
...
int checkBranchType(char* line){    //返回值含义， 0：不是分支语句，1：if语句，2：else if语句， 3：else语句  4：while  5：for   6：}符号
    int length=strlen(line);
    if(strcmp(line,"}")==0){
        return 6;
    }
    if(length>7 && strcmp(substring(line,0,6),"while(")==0 && strcmp(substring(line,length-2,length),"){")==0){
        return 4;    //判断是不是while(){
    }
    if(length>5 && strcmp(substring(line,0,4),"for(")==0 && strcmp(substring(line,length-2,length),"){")==0){
        //printk("find for express\n");
        return 5; //判断是不是for(){
    }
    if(*line=='i' && *(line+1)=='f' && *(line+2)=='('){   //判断是不是if(){
        int i=3;
        int find=0;
        for(;i<length;i++){
            if(*(line+i)==')'){
        ...
        if(find){
            if(i+2==length && *(line+i+1)=='{'){
                return 1;
            }
        }
    }
    char* dest;
    if(length==6 && strcmp(line,"}else{")==0){
        return 3;
    }
    if(length>9){
        //printk("prefix: %s\n",substring(dest,line,0,8));
        if(strcmp(substring(line,0,8),"}elseif(")==0){
            int i=8;
            int find=0;
            for(;i<length;i++){
                if(*(line+i)==')'){
...
void getVariableAndCompareMode(struct Line* line){
    char* tmp;
    if(line->type==5){
        tmp=strchr(line->briefCmd,';');
        if(tmp==NULL){
            line->error=1;
            return;
        }
        tmp=substring(tmp,1,strlen(tmp));
        int mark=-1;
        int i=0;
        while(i<strlen(tmp)){
            if(*(tmp+i)==';'){
        ...
        line->expr=substring(tmp,mark+1,strlen(tmp)-2);
        tmp=substring(tmp,0,mark);
        //printk("tmp express: %s\n",line->expr);
        //printk("between ; is %s\n",tmp);
    }
    if(line->type==1){
        tmp=substring(line->briefCmd,3,strlen(line->briefCmd)-2);
    }else if(line->type==2){
        tmp=substring(line->briefCmd,8,strlen(line->briefCmd)-2);
    }else if(line->type==4){   // while(){
        tmp=substring(line->briefCmd,6,strlen(line->briefCmd)-2);
    }

    int length=strlen(tmp);
    int i=0;
       int mark1=-1;     //标记比较运算符开始的位置
        int mark2=-1;     //标记比较运算符结束的位置
        for(;i<length;i++){
            char c1=*(tmp+i);
            if(c1=='>'){
                mark1=i;
                mark2=i;
                line->compareMode=1;    //表示解析出>运算符
                if((i+1<length) && *(tmp+i+1)=='='){
                    mark2=i+1;
                    line->compareMode=2;  //表示解析出>=运算符
                ...
                mark1=i;
                mark2=i;
                line->compareMode=3;    //表示解析出<运算符
                if((i+1<length) && *(tmp+i+1)=='='){
                    mark2=i+1;
                    line->compareMode=4;  //表示解析出<=运算符

                }
                break;
            }else if(c1=='=' && (i+1<length) && *(tmp+i+1)=='=' ){
                mark1=i;
                mark2=i+1;
                line->compareMode=5;     //解析出==运算符

                break;
            }else if(c1=='!' && (i+1<length) && *(tmp+i+1)=='=' ){
                mark1=i;
                mark2=i+1;
                line->compareMode=6;     //解析出!=运算符

                break;
            }
       }
        if(mark1==-1 || mark1==0 || length-mark2-1 ==0){
                line->error=1;
                return;
        }

        //否则代表比较符前后确实有两个变量；
        line->variableNames[0]=substring(tmp,0,mark1);
        line->variableNames[1]=substring(tmp,mark2+1,length);
...
void getValueOfVariables(struct Line* line){
    ...
    if(*(line->variableNames[0])=='$'){
            line->valuesOfVariable[0]=(char*)malloc(sizeof (char)*128);

            int valueLength1=__tshellVarGetRt(substring(line->variableNames[0],1,strlen(line->variableNames[0])),line->valuesOfVariable[0],128);
            if(valueLength1==-1){
                //printk("get value1 failed\n");
                line->error=1;
                return;
            }
        }else if(*(line->variableNames[0])=='\"'){

           line->valuesOfVariable[0]=substring(line->variableNames[0],1,strlen(line->variableNames[0])-1);
        }else{
           line->valuesOfVariable[0]=substring(line->variableNames[0],0,strlen(line->variableNames[0]));
    }
    if(*(line->variableNames[1])=='$'){
        line->valuesOfVariable[1]=(char*)malloc(sizeof (char)*128);
                int valueLength2=__tshellVarGetRt(substring(line->variableNames[1],1,strlen(line->variableNames[1])),line->valuesOfVariable[1],128);
                if(valueLength2==-1){
                    //printk("get value2 failed\n");
                    line->error=1;
                    return;
                }
            }else if(*(line->variableNames[1])=='\"'){

               line->valuesOfVariable[1]=substring(line->variableNames[1],1,strlen(line->variableNames[1])-1);
            }else{
               line->valuesOfVariable[1]=substring(line->variableNames[1],0,strlen(line->variableNames[1]));
    }
    char* numbers="0123456789";
            int i=0;
            int length1=strlen(line->valuesOfVariable[0]);
            int length2=strlen(line->valuesOfVariable[1]);
            for(;i<length1;i++){
                if(strchr(numbers,*(line->valuesOfVariable[0]+i))==NULL){
                    //printk("position %d is not digit\n",i);
                    line->valueIsNum[0]=0;
                    break;
                }
            }
            i=0;
            for(;i<length2;i++){
                        if(strchr(numbers,*(line->valuesOfVariable[1]+i))==NULL){
                            //printk("position %d is not digit\n",i);
                            line->valueIsNum[1]=0;
                            break;
                        }
...
void compareValues(struct Line* line){
       ...
       if(line->valueIsNum[0]==0 || line->valueIsNum[1]==0){
           //printk("one is not num\n");
           if(line->compareMode<5){
               line->error=1;
               return;
           }
       }

       if(line->valueIsNum[0]==1 && line->valueIsNum[1]==1){
           //printk("compare in num\n");
          if(line->compareMode==1 && atoi(line->valuesOfVariable[0])>atoi(line->valuesOfVariable[1])){
              line->executable=1;
          }else if(line->compareMode==2 && atoi(line->valuesOfVariable[0])>=atoi(line->valuesOfVariable[1])){
              line->executable=1;
          }else if(line->compareMode==3 && atoi(line->valuesOfVariable[0])<atoi(line->valuesOfVariable[1])){
              line->executable=1;
          }else if(line->compareMode==4 && atoi(line->valuesOfVariable[0])<=atoi(line->valuesOfVariable[1])){
              line->executable=1;
          }else if(line->compareMode==5 && atoi(line->valuesOfVariable[0])==atoi(line->valuesOfVariable[1])){
              line->executable=1;
          }else if(line->compareMode==6 && atoi(line->valuesOfVariable[0])!=atoi(line->valuesOfVariable[1])){
              line->executable=1;
          }
       }else{
          //printk("compare in string\n");
          if(line->compareMode==5 && strcmp(line->valuesOfVariable[0],line->valuesOfVariable[1])==0){
              line->executable=1;
          }else if(line->compareMode==6 && strcmp(line->valuesOfVariable[0],line->valuesOfVariable[1])!=0){
              //printk("get != success\n");
              line->executable=1;
          }
      }
...
static INT  __tshellFsCmdShfile (INT  iArgC, PCHAR  ppcArgV[])
{
    ...
    if (pcCmdBuffer == LW_NULL) {
        ...
        //result.compareMode的含义 1.>  2.>=   3.<    4.<=    5.==    6.!=

    //printk("cd shfile function\n");
    //printk("fileName: %s\n",ppcArgV[1]);
    if(strcmp(ppcArgV[1],"/etc/startup.sh")==0){
        do {
    ...
    struct Line lines[128];
    int cmdCount=0;
    int stack[10];
    int stackPointer=-1;
    char* tmpCmd=fgets(pcCmdBuffer, LW_CFG_SHELL_MAX_COMMANDLEN, fileShell);
    while(tmpCmd){
        lines[cmdCount]=parseLineStruct(tmpCmd);

        if(lines[cmdCount].type!=0){
                    if(lines[cmdCount].type==2 || lines[cmdCount].type==3 || lines[cmdCount].type==6 ){
                        lines[stack[stackPointer]].closed=cmdCount;
                        stackPointer--;
                    }
                    if(lines[cmdCount].type!=6){
                    stackPointer++;
                    stack[stackPointer]=cmdCount;
                    }
        }
        cmdCount++;
        tmpCmd=fgets(pcCmdBuffer, LW_CFG_SHELL_MAX_COMMANDLEN, fileShell);
    }
        ...
        for(;i<cmdCount;i++){
            printk("line %d:%s\n",i,lines[i].briefCmd);
            printk("type: %d, closed at:%d\n",lines[i].type,lines[i].closed);
            printk("\n\n");
        }*/
    int cmdPointer=0;
    int whileEndStack[10]={-1};
    int whileBeginStack[10]={-1};
    int whileStackPointer=-1;
    int forBeginStack[10]={-1};
    int forEndStack[10]={-1};
    int forStackPointer=-1;
    while(cmdPointer<cmdCount){
        int type=lines[cmdPointer].type;
        lines[cmdPointer].executable=0;
        //printk("cmdPointer: %d\n",cmdPointer);
        if(type==0){
            API_TShellExec(lines[cmdPointer].originalCmd);
        }else if(type==1 || type==2 || type==5){
            //lines[cmdPointer].executable=0;
            if(type==5){
                if(forStackPointer==-1 || forBeginStack[forStackPointer]!=cmdPointer){
                    //printk("forStackPointer increased to %d\n",forStackPointer);
                    forStackPointer++;
                }
            }

            if(lines[cmdPointer].blocked==1){
                //printk("blocked\n");
                lines[cmdPointer].blocked=0;
                cmdPointer=lines[cmdPointer].closed;
                continue;
            }

            //printk("inner str: %s\n",lines[cmdPointer].briefCmd);
            getVariableAndCompareMode(&lines[cmdPointer]);
            if(lines[cmdPointer].error==1){
                printk("syntax error\n");
                break;
            }
            //printk("variableName1 is %s , variableName2 is %s\n",lines[cmdPointer].variableNames[0],lines[cmdPointer].variableNames[1]);
            getValueOfVariables(&lines[cmdPointer]);
            if(lines[cmdPointer].error==1){
                printk("cannot get value of variable\n");
                break;
            }
            compareValues(&lines[cmdPointer]);
            if(lines[cmdPointer].error==1){
                           printk("cannot compare number with string\n");
                           break;
            }
            if(lines[cmdPointer].executable==1){
                if(type==5){

                    forBeginStack[forStackPointer]=cmdPointer;
                    forEndStack[forStackPointer]=lines[cmdPointer].closed;
                }else{
                    int closed=lines[cmdPointer].closed;
                    while(closed!=-1){
                        lines[closed].blocked=1;
                        closed=lines[closed].closed;
                    }
                }
            }else{
                //printk("condition not satisfied\n");
                cmdPointer=lines[cmdPointer].closed;
                if(type==5){
                    forStackPointer--;
                }
            ...
            //printk("value1 is %s, value2 is %s\n",lines[cmdPointer].valuesOfVariable[0],lines[cmdPointer].valuesOfVariable[1]);
        }else if(type==3){
                        //lines[cmdPointer].executable=0;
                        if(lines[cmdPointer].blocked==1){
                            lines[cmdPointer].blocked=0;
                        ...
                        if( whileStackPointer==-1 || whileBeginStack[whileStackPointer]!=cmdPointer){
                        whileStackPointer++;
                        //printk("whileStackPointer increased: %d\n",whileStackPointer);
                        }
                        //lines[cmdPointer].executable=0;
                        getVariableAndCompareMode(&lines[cmdPointer]);
                        if(lines[cmdPointer].error==1){
                        ...
                        //printk("value1 is %s, value2 is %s\n",lines[cmdPointer].valuesOfVariable[0],lines[cmdPointer].valuesOfVariable[1]);
                        //printk("executable is %d\n",lines[cmdPointer].executable);
                        compareValues(&lines[cmdPointer]);
                        if(lines[cmdPointer].error==1){
                                    printk("cannot compare number with string\n");
                                    break;
                        ...
                        if(lines[cmdPointer].executable==1){
                                        ...
                                        whileEndStack[whileStackPointer]=lines[cmdPointer].closed;
                                        whileBeginStack[whileStackPointer]=cmdPointer;
                        }else{
                                        //printk("condition not satisfied\n");
                                        cmdPointer=lines[cmdPointer].closed;
                                        whileStackPointer--;
                                        //printk("whileStackPointer reduced to %d\n",whileStackPointer);
                                        continue;
                        }
        }
        cmdPointer++;
        if(whileStackPointer>-1 && cmdPointer==whileEndStack[whileStackPointer]+1){
            //printk("end while\n");
            cmdPointer=whileBeginStack[whileStackPointer];
        }
        if(forStackPointer>-1 && cmdPointer==forEndStack[forStackPointer]+1){
            //printk("end for\n");
            cmdPointer=forBeginStack[forStackPointer];
            API_TShellExec(lines[cmdPointer].expr);
        }
    }
    }
